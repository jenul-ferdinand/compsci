...
export { initialState, reduceState, Teleport, Rotate, Shoot, Tick, Thrust }
...
class Teleport implements Action {
    public readonly p: [number,number]
    constructor(destination: [number,number]) {
        this.p = destination.map(x=>x*Constants.CanvasSize/2) as [number,number]
    }
    apply = (s:State) => ({...s,
        // Exercise 3:
        // ship: { ...s.ship, pos: torusWrap(s.ship.pos.add(new Vec(0,-1).rotate(s.ship.angle).scale(200))) }
        // Exercise 4:
        ship: { ...s.ship, pos: new Vec(this.p[0], this.p[1]) }
    })
}
