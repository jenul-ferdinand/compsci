 ...
 */
import { fromEvent, interval, Subscription, merge, zip, Observable } from 'rxjs';
import { map, filter, scan } from 'rxjs/operators';
import { Key, Event, Action, State } from './types'
import { RNG, IMPLEMENT_THIS } from './util';
import { Tick, Rotate, Thrust, Shoot, Teleport, initialState, reduceState } from './state';
...
function asteroids() {
  ...
  const
    ...
    rng$ = (seed:number)=>key$('keydown','Enter').pipe(scan(RNG.hash,seed),map(RNG.scale)),
    teleport$         = zip(rng$(123),rng$(321)).pipe(
      map(randomPair => new Teleport(randomPair)))
  ...
  const action$: Observable<Action> = //IMPLEMENT_THIS;
          merge(gameClock$
            ...
            , startThrust$, stopThrust$, shoot$, teleport$);
