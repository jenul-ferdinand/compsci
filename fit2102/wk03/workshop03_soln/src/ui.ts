...
import { tap, scan, map, reduce, mergeMap, switchMap, zipWith  } from 'rxjs/operators';
import { Card, deal$, shoe$, deckSize, Count } from './exercises'
 ...
 */
const hitme$ : (_:Observable<Count>) => Observable<Count>
  = count$ => fromEvent(button, 'click').pipe(
      switchMap(
        _=> {
          console.log("switch!")
          return interval(500).pipe(zipWith(count$))
        }
      ),
      map((([_,c])=>c))
    )
  ...
  .subscribe(count=>{
    if(count.card) {
      ...
      cardDisplay.innerHTML
        = `Card dealt: ${count.cardsDealt}<br><span id="cardspan" style="color:${cardColour}">${count.card.suit}${count.card.rank}</span>`
    }
    ...
    const countColour = !count.trueCount ? "antiquewhite"
                      : count.trueCount < 0 ? "#93c5df" 
                      : "#f4a581" // > 0
...
function addBar(count: Count, colour: string) {
  ...
  , y = !count.trueCount ? yMid - 0.5
      : count.trueCount < 0 ? yMid
      : yMid - height // >0
  Object.entries({ 
    x: String(x),
    y: String(y),
