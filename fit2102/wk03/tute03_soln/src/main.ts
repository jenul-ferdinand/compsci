...
function mousePosObservable() {
    ...
    // Source Observable, which event do we want to listen to?
    // https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent
    const source$ = fromEvent<MouseEvent>(document, "mousemove");

    const xy$ = source$.pipe(
        map(({ clientX, clientY }) => ({ x: clientX, y: clientY })),
    );

    const [add, remove] = partition(xy$, ({ x }) => x > 400);

    xy$.subscribe(({ x, y }) => (elem.textContent = `${x},${y}`));
    add.subscribe(() => elem.classList.add("highlight"));
    remove.subscribe(() => elem.classList.remove("highlight"));
...
function animatedRect() {
    ...
    // The timer Observable will only emit once, avoiding memory leaks
    const stop$ = timer(1000);

    const move$ = source$
        .pipe(
            takeUntil(stop$),

            // Accumulate changing state
            // We need to type prevX: number otherwise Typescript
            // will infer it as the literal 100
            scan((prevX: number, _) => prevX + 1, startProps.x),
        )
        .subscribe(newX => rect.setAttribute("x", String(newX)));
}
...
function keyboardControl() {
     ...
     *
     * @param keyCode
     * @param IMPLEMENT_THIS Add whatever props you need to identify
     *  the keypress and the associated change in state.
     * @returns Observable stream that indicates changes in state for
     *  the particular keypress
     */
    const fromKey = (keyCode: string, axis: "x" | "y", amount: number) =>
        key$.pipe(
    ...
    merge(left$, down$, up$, right$)
        .pipe(
            ...
            scan(
                (state, { axis, amount }) => {
                    return { ...state, [axis]: state[axis] + amount };
                },
                {
                    x: startProps.x,
                    y: startProps.y,
